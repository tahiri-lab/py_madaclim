...
def get_taxon_key_by_species_match(
    name: Optional[str] = None, return_full_on_match=False, **kwargs: Union[str, bool]
) -> Union[dict, int]:
    """
    ...
    if name is not None and not isinstance(name, str):
        raise TypeError(
            f"'name' must be a str if specified, but received {type(name).__name__}"
        )
    ...
    for param, value in params.items():
        if param not in possible_params:
            raise ValueError(
                f"Argument '{param}' is not valid. It must be one of {possible_params}"
            )
        if param == "strict" and not isinstance(value, bool):
            raise TypeError(
                f"The argument 'strict' must be of type bool, but received {type(value).__name__}"
            )
        if param != "strict" and not isinstance(value, str):
            raise TypeError(
                f"Keyword arguments values must be of type str, but received {type(value).__name__} for {param}"
            )
    ...
    if len(params) == 0:
        raise ValueError(
            f"Provide at least one match param argument from {possible_params} if 'name' is not provided."
        )
...
def search_occ_mdg_valid_coordinates(
    taxon_key: int, year_range: Optional[tuple[int]] = None
) -> list:
    """
    ...
    if not isinstance(taxon_key, int):
        raise TypeError(
            f"'taxon_key' must be an int, but received {type(taxon_key).__name__}"
        )
    ...
    if year_range is not None:
        ...
        if year_range[0] > year_range[1]:
            raise ValueError(
                "First year element in range must be smaller than the second."
            )
    ...
    params = {
        "gadmGid": "MDG",
        "hasCoordinate": "true",  # "has_coordinate": "true",
        "hasGeospatialIssue": "false",  # "has_geospatial_issue": "false",
        "taxonKey": taxon_key,  # "taxon_key": taxon_key,
    ...
    if data["count"] == 0:
        ...
        if year_range:
            print(
                f"Fetching all {data['count']} occurences in year range {year_range[0]}-{year_range[1]}..."
            )
        else:
            print(
                f"Fetching all {data['count']} occurences accross all possible years..."
            )
...
def request_occ_download_mdg_valid_coordinates(
    ...
    try:
        ...
        raise TypeError(
            "'dotenv_filepath' must be a valid type for the pathlib.Path constructor (str or system-specific pathlib.Path/PurePath)"
        )
    ...
    if not all([key in config.keys() for key in dotenv_keys]):
        raise ValueError(
            f"Missing credentials keys in '.env' file. The file must contain both {dotenv_keys} keys"
        )
    ...
    try:
        response = requests.post(
            url=url, json=payload, auth=(gbif_username, gbif_password)
        )
...
def download_extract_read_occ(
    ...
    if not isinstance(download_id, str):
        raise TypeError(
            f"'download_id' must be a str, but received {type(download_id).__name__}."
        )
    ...
    if target_dir is None:
        ...
        try:
            ...
            raise TypeError(
                "'target_dir' must be a valid type for the pathlib.Path constructor (str or system-specific pathlib.Path/PurePath)"
            )
    ...
    with open(target_dir / zipfile_name, "wb") as f:
        ...
        for n, chunk in enumerate(response.iter_content(chunk_size=chunk_size)):
            ...
            now_time = time.time()
            current_speed = (
                (n * chunk_size) / (now_time - start_time) / (1024 * 1024)
            )  # current_speed is in MB/s
            print(
    ...
    with ZipFile(target_dir / zipfile_name, mode="r") as zObj:
        ...
        print(
            f"Extracting all {len(zipped_files)} files to target location: .../{download_container.name}/"
        )
    ...
    if download_format not in possible_formats:
        raise ValueError(
            f"download_format from parsed HTTP response is not one of {possible_formats}"
        )
    ...
    if download_format == "DWCA":
        with DwCAReader(target_dir / zipfile_name) as dwca:
            ...
            print(
                f"Read and saved core data into pandas df: {dwca.descriptor.core.file_location}"
            )  # 'occurrence.txt'
        ...
        if len(csv_files) == 0:
            ...
            print(
                f"BEWARE! More than 1 '.csv' file in the extracted {zipfile_name}. Could not read into pandas df"
            )
            df = None
        else:
