...
class Multi:
    ...
    def processing_large_data(self):
        ...
        p = Process(
            target=self.execute_once, args=([self.args.pop(0)])
        ...
        print(
            "\033[B" * self.rewrite[False], flush=True
        ...
        while len(self.args) != 0:
            if (self.tasks.value < self.max_allowed.value) & (
                self.nb_allowed.value >= 1
            ):
                p = Process(
                    target=self.execute_once, args=([self.args.pop(0)])
        ...
        print("\r", end="")
        print(
            "\033[B" * self.rewrite[True], flush=True
        )  # "\033[B" acts as a reverse \n, return the pointer one line up
        print(
            "Completed with ", str(self.amount - self.finished.value), " errors\n"
    ...
    def mem_update(self):
        ...
        for child in self.processes:
            try:  # in a try/except because processes ID are never removed from the list
                mem = psutil.Process(child).memory_full_info()[
                    8
        ...
        if self.nb_allowed.value < 1:
            self.nb_allowed.value = (
                1  # Need to at least be able to start a single process
            )
    ...
    def terminal_update(self, memBlock):
        ...
        if memBlock:  # block of prints used only by processing_large_data()
            print(
                "Available memory: ",
                round(self.mem_a.value / 10000000) / 100,
                "/",
                round(self.mem_t.value / 10000000) / 100,
                "Gb        ",
                end="\n",
                flush=True,
            )
            ...
            print(
                "Time for one:     ",
                round(self.time_for_one.value * 10) / 10,
                " seconds               ",
                flush=True,
            )
            print("---")
        ...
        print(
            "\033[A" * self.rewrite[memBlock], flush=True
        )  # "\033[B" acts as a reverse \n, return the pointer one line up
    ...
    def processing_small_data(self):
        ...
        for a in range(len(self.args)):
            p = Process(
                target=self.execute_small, args=([self.args.pop(0)])
            )  # Multiprocess runs
            self.process_list.append(p)
            p.start()
