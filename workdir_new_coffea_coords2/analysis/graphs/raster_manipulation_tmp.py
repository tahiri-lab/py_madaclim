...
class _PlotConfig:
    ...
    def __init__(self, plot_element_args: Optional[dict] = None) -> None:
        ...
        self._imshow_args = self._classify_args(
            prefix="imshow_", args=self._plot_element_args
        )
        self._cax_args = self._classify_args(
            prefix="cax_", args=self._plot_element_args
        )
        self._histplot_args = self._classify_args(
            prefix="histplot_", args=self._plot_element_args
        )
        self._subplots_args = self._classify_args(
            prefix="subplots_", args=self._plot_element_args
        )
        self._rasterpoint_args = self._classify_args(
            prefix="rasterpoint_", args=self._plot_element_args
        )
        self._vline_args = self._classify_args(
            prefix="vline_", args=self._plot_element_args
        )
        self._barplot_args = self._classify_args(
            prefix="barplot_", args=self._plot_element_args
        )
    ...
    def _classify_args(self, prefix: str, args: dict):
        ...
        if prefix not in self.POSSIBLE_PREFIXES:
            raise ValueError(
                f"Specified 'prefix' must be one of {self.POSSIBLE_PREFIXES}"
            )
        ...
        for k, v in args_dict.items():
            ...
            if k.startswith(prefix):
                if not re.match(r"\w+_", k):
                    raise ValueError(
                        f"'args' must be prefixed by at least 1 alphanum char and an underscore (i.e. 'prefix_argument'): {k}"
                    )

                arg_prefix = (
                    k.split("_", 1)[0] + "_"
                )  # First underscore encounter for args with native underscores
                if arg_prefix not in self.POSSIBLE_PREFIXES:
                    raise ValueError(
                        f"{arg_prefix} (derived from {k}) must be one of {self.POSSIBLE_PREFIXES}"
                    )
...
class _LayerPlotter:
    ...
    def __init__(
        self,
        layer_num: int,
        madaclim_layers: py_madaclim.info.MadaclimLayers,
        plot_args: dict,
    ) -> None:
        """
    ...
    def plot_layer2(
        ...
        if subplots_figsize:  # Better exception hints than matplotlib
            ...
            for ele in subplots_figsize:
                if not isinstance(ele, int):
                    raise TypeError(
                        f"{ele} is not an int. 'subplots_figsize' is a tuple of integers."
                    )
        ...
        layer_units = (
            ...
            else layer_info["units"]
        )  # Simplify xlabel for categorical units
        ...
        geoclim_type = [
            geotype
            for geotype in geoclim_types
            if self.layer_num
            in self._madaclim_layers.select_geoclim_type_layers(geotype)[
                "layer_number"
            ].values
        ][0]
        chosen_raster = (
        ...
        with rasterio.open(chosen_raster) as raster:
            ...
            if layer_info["is_categorical"]:
                ...
                subplots_figsize = (
                    (20, 10) if subplots_figsize is None else subplots_figsize
                )
                fig, axes = plt.subplots(
                    nrows=subplots_nrows, ncols=subplots_ncols, figsize=subplots_figsize
                )

                # Custom categorical palette for over 20 categories
                categ_combinations = self._madaclim_layers.get_categorical_combinations(
                    self.layer_num
                )
                categ_combinations = categ_combinations[
                    self._madaclim_layers.get_layers_labels(self.layer_num)[0]
                ]
                categ_combinations = dict(
                    sorted(categ_combinations.items(), key=lambda item: item[0])
                )  # Get categ dict {<numerical_val>:<category>}
                if len(categ_combinations) > 20:
                    ...
                    colors = sns.color_palette(
                        palette="tab20", n_colors=len(categ_combinations)
                    )

                # Exclude masked values
                unmasked_values = [
                    v for v in categ_combinations.keys() if v is not np.ma.masked
                ]
                unmasked_colors = colors[: len(unmasked_values)]

                # Make sure categories are sorted and mapped to colors accordingly
                sorted_categ_combinations = dict(
                    sorted(zip(unmasked_values, unmasked_values))
                )
                categ_colors = {
                    category: color
                    for category, color in zip(
                        sorted_categ_combinations.values(), unmasked_colors
                    )
                }
                ...
                categ_labels = [
                    categ_combinations[cat] for cat in sorted_categ_combinations.keys()
                ]
                legend_elements = [
                    Patch(facecolor=color, edgecolor=color, label=category)
                    for color, category in zip(categ_colors.values(), categ_labels)
                ]
                axes[0].legend(
                ...
                mapped_categ_combinations = [
                    (
                        sorted_categ_combinations[c]
                        if c in sorted_categ_combinations
                        else None
                    )
                    for c in categ
                ]
                if None in mapped_categ_combinations:
                    raise ValueError(
                        "Keys from `get_categorical_combinations` does not match raster values"
                    )
                ...
                df_cat = pd.DataFrame(
                    {
                        "category": categ,
                        "mapped_category": mapped_categ_combinations,
            ...
            # Continous data for multi plots
                ...
                subplots_figsize = (
                    (12, 6) if subplots_figsize is None else subplots_figsize
                )
                fig, axes = plt.subplots(
                    nrows=subplots_nrows, ncols=subplots_ncols, figsize=subplots_figsize
                )
                ...
                imshow_vmin = plot_cfg.imshow_args.pop(
                    "vmin", np.nanmin(band_data.squeeze())
                )
    ...
    def _validate_madaclim_layers(
        self, madaclim_layers: py_madaclim.info.MadaclimLayers
    ) -> py_madaclim.info.MadaclimLayers:
        if not isinstance(madaclim_layers, py_madaclim.info.MadaclimLayers):
            raise TypeError(
                "'madaclim_layers' must be of an instance of py_madaclim.info.MadaclimLayers"
            )

        if madaclim_layers.clim_raster is None or madaclim_layers.env_raster is None:
            raise ValueError(
                "The MadaclimLayers instance must have defined 'clim_raster' and 'env_raster attributes."
            )
...
class MadaclimRasters:
    ...
    def plot_layer(
        self, layer: Union[str, int], **kwargs
    ) -> Tuple[matplotlib.figure.Figure, List[matplotlib.axes.Axes]]:
        """
        ...
        madaclim_info = MadaclimLayers(
            clim_raster=self.clim_raster, env_raster=self.env_raster
        )
        ...
        possible_layers_desc_format = madaclim_info.get_layers_labels(
        ...
        if (
            layer in possible_layers_num_format or layer in possible_layers_desc_format
        ):  # Check layer_<num> or descriptive format
            ...
            try:
                ...
                raise TypeError(
                    "layer must be either a single int value or a string that can be converted to an int"
                )
        ...
        if not min_layer <= layer_num <= max_layer:
            raise ValueError(
                f"layer_number must fall between {min_layer} and {max_layer}. {layer_num=} is not valid."
            )

        # Use LayerPlotter instance helper class to handle customization + plot layer
        plotter = _LayerPlotter(
            layer_num=layer_num, madaclim_layers=madaclim_info, plot_args=kwargs
        )
    ...
    def _validate_raster(self, raster: Union[str, pathlib.Path]) -> pathlib.Path:
        ...
        warnings.filterwarnings(
            "error", category=rasterio.errors.NotGeoreferencedWarning
        )
...
class MadaclimPoint:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._mada_geom_point = self._construct_point(
            latitude=self.latitude, longitude=self.longitude, source_crs=self.source_crs
        )
        ...
        self.__initial_attributes = (
            self.__dict__.copy()
        )  # For dealing with newly set attr post-instantiation
    ...
    def longitude(self, value: float):
        if hasattr(self, "_longitude") and hasattr(self, "_latitude"):
            self._longitude, _ = self._validate_lonlat(
                longitude=value, latitude=self._latitude
            )
            self._update_mada_geom_point()
    ...
    def latitude(self, value: float):
        if hasattr(self, "_longitude") and hasattr(self, "_latitude"):
            _, self._latitude = self._validate_lonlat(
                longitude=self._longitude, latitude=value
            )
            self._update_mada_geom_point()
        ...
        for k, v in self.__dict__.items():
            ...
            if k not in [
                ...
                if k == "_source_crs":  # Display EPSG code for readability
                    ...
                    k == "_sampled_layers"
                ):  # Display number of sampled layers or status of sampling
                    ...
                    k == "_nodata_layers"
                ):  # Display number of nodata layers or status of sampling
                    if self._sampled_layers:
                        show_attributes["_len(nodata_layers)"] = (
                            f"{len(v)} layer(s)"
    ...
    def get_default_source_crs(as_epsg: bool = True) -> Union[pyproj.crs.crs.CRS, int]:
        ...
        """
        source_crs_val = inspect.getfullargspec(MadaclimPoint).defaults[
            0
        ]  # only 1 default args
        if as_epsg:
    ...
    def sample_from_rasters(
        ...
        Examples:
            ...
            Warning message for NaN in the data extracted
            ...
            Updated attributes post-sampling
        ...
        if mada_rasters.clim_crs != Constants.MADACLIM_CRS:
            raise ValueError(
                f"The provided clim_raster's CRS does not corresponds to Madaclim db's expected crs: {Constants.MADACLIM_CRS}"
            )

        if mada_rasters.env_crs != Constants.MADACLIM_CRS:
            raise ValueError(
                f"The provided env_raster's CRS does not corresponds to Madaclim db's expected crs: {Constants.MADACLIM_CRS}"
            )

        # Create a MadaclimLayers instance to get layers labels and validate layers to sample
        madaclim_info = MadaclimLayers(
            clim_raster=mada_rasters.clim_raster, env_raster=mada_rasters.env_raster
        )
        all_layers_df = madaclim_info.all_layers
        ...
        if isinstance(layers_to_sample, list):
            ...
            layers_num_format = all(
                [
                    layer_label in possible_layers_num_format
                    for layer_label in layers_to_sample
                ]
            )
            layers_desc_format = all(
                [
                    layer_label in possible_layers_desc_format
                    for layer_label in layers_to_sample
                ]
            )
            ...
            if layers_num_format or layers_desc_format:
                ...
                layers_numbers = [
                    int(layer_label.split("_")[1]) for layer_label in layers_to_sample
                ]
            ...
            # layers_to_sample as list of ints
                ...
                try:
                    ...
                    raise TypeError(
                        "layers_to_sample must be either a single int value or a string that can be converted to an int, or a list of int values or strings that can be converted to int values"
                    )
        ...
        # Single layers_to_sample type check
            ...
            if layers_to_sample == "all":  # Get all layers as default
                ...
                layers_to_sample in possible_layers_num_format
                or layers_to_sample in possible_layers_desc_format
            ):  # Check layer_<num> or desc str format
                ...
                try:
                    ...
                    raise TypeError(
                        "layers_to_sample must be either a single int value or a string that can be converted to an int"
                    )
        ...
        for layer_number in layers_numbers:
            if not min_layer <= layer_number <= max_layer:
                raise ValueError(
                    f"layer_number must fall between {min_layer} and {max_layer}. {layer_number=} is not valid."
                )
        ...
        geoclim_types = ["clim", "env"]
        geoclim_layer_ranges = {
            geoclim_type: madaclim_info.select_geoclim_type_layers(geoclim_type)[
                "layer_number"
            ].to_list()
            for geoclim_type in geoclim_types
        }

        clim_raster_sample_info["layers"] = [
            layer_num
            for layer_num in layers_numbers
            if layer_num in geoclim_layer_ranges["clim"]
        ]
        clim_raster_sample_info["bands"] = madaclim_info.get_bandnums_from_layers(
            clim_raster_sample_info["layers"]
        )
        has_clim_data = True if clim_raster_sample_info["layers"] else False

        env_raster_sample_info["layers"] = [
            layer_num
            for layer_num in layers_numbers
            if layer_num in geoclim_layer_ranges["env"]
        ]
        env_raster_sample_info["bands"] = madaclim_info.get_bandnums_from_layers(
            env_raster_sample_info["layers"]
        )
        ...
        if not isinstance(self._mada_geom_point, shapely.geometry.point.Point):
            raise TypeError(
                "The 'mada_geom_point' attribute must be a shapely.geometry.point.Point object."
            )
        ...
        print(
            "\n"
            + "#" * 40
            + f" \033[1mExtracting data for: {self._specimen_id}\033[0m "
            + "#" * 40
        )
        ...
        if clim_raster_sample_info["layers"]:
            ...
            with rasterio.open(mada_rasters.clim_raster) as clim_raster:
                ...
                with tqdm(
                    ...
                    for layer_num, band_num in zip(
                        clim_raster_sample_info["layers"],
                        clim_raster_sample_info["bands"],
                    ):
                        ...
                        pbar.set_description(
                            f"Extracting layer {layer_num}: {layer_metadata['layer_description'].values[0]}"
                        )
                        pbar.update()
                        ...
                        data = list(
                            clim_raster.sample(
                                [(self._mada_geom_point.x, self._mada_geom_point.y)],
                                indexes=band_num,
                            )
                        )[0]
                        ...
                        layer_label = (
                            ...
                            if not layer_info
                            else madaclim_info.get_layers_labels(
                                layer_num, as_descriptive_labels=True
                            )[0]
                        )
                        ...
                        if (
                            data[0] == nodata_clim
                        ):  # Save layers where nodata at specimen location
        ...
        if len(nodata_layers) > 0:  # No raising exception, just warning print
            print(
                f"BEWARE! {len(nodata_layers)} layer(s) contain a nodata value at the specimen location"
            )
        ...
        nodata_val = (
            nodata_clim if has_clim_data else nodata_env
        )  # OK because clim_nodata == env_nodata

        converted_nan_sampled_layers = {  # Replace nodata int with NaN
            layer_label: np.nan if val == nodata_val else val
            for layer_label, val in sampled_layers.items()
        }
    ...
    def binary_encode_categorical(self) -> None:
        ...
        if not self._sampled_layers:
            raise ValueError(
                "Raster data have not been sampled yet. Use `sample_from_rasters` method prior."
            )

        # Check if sampled_layers contains categorical data
        madaclim_info = MadaclimLayers()
        possible_categ_labels = madaclim_info.get_categorical_combinations()
        possible_descriptive_categ_labels = madaclim_info.get_categorical_combinations(
            as_descriptive_keys=True
        )

        sampled_layers_labels = set(self._sampled_layers.keys())
        categ_layers = set(possible_categ_labels.keys()) | set(
        ...
        for layer in sorted(intersect_layers):
            ...
            categories_dict = possible_categ_labels.get(
                layer, {}
            ) or possible_descriptive_categ_labels.get(layer, {})
        ...
        self._encoded_categ_layers = (
            encoded_categ  # Overridden `settr` will update `gdf`
        )
    ...
    def plot_on_layer(self, layer: Union[str, int], **kwargs) -> None:
        ...
        def layer_name_range_validation(
            ...
            possible_layers_desc_format = mada_info.get_layers_labels(
                as_descriptive_labels=True
            )
            ...
            if (
                layer in possible_layers_num_format
                or layer in possible_layers_desc_format
            ):  # Check layer_<num> or descriptive format
                layer_num = int(layer.split("_")[1])
            else:
                try:
        ...
        if (
            self._MadaclimPoint__clim_raster is None
            or self._MadaclimPoint__env_raster is None
        ):
        ...
        mada_rasters = MadaclimRasters(
            self._MadaclimPoint__clim_raster, self._MadaclimPoint__env_raster
        )
        mada_info = layer_name_range_validation(
            mada_rasters.clim_raster, mada_rasters.env_raster, layer
        )
        ...
        detailed_layer_label = mada_info.get_layers_labels(
            layer_number, as_descriptive_labels=True
        )[0]

        if not (
            simple_layer_label in self._sampled_layers
            or detailed_layer_label in self._sampled_layers
        ):
            raise ValueError(
        ...
        rasterpoint_args = {
            ...
            "legend_kwds": plot_cfg.rasterpoint_args.pop(
                "legend_kwds", {"loc": (0.1, 0.9)}
            ),
            "customlabel": plot_cfg.rasterpoint_args.pop(
                "customlabel", self._specimen_id
            ),
        }
        ...
        if rasterpoint_args["legend"]:
            ...
            axes[0].legend(
                handles=[legend_handle], loc=rasterpoint_args["legend_kwds"]["loc"]
            )
        ...
        vline_args = {
            ...
            "linestyle": plot_cfg.vline_args.pop(
                "linestyle", rasterpoint_args["linestyle"]
            ),
            "linewidth": plot_cfg.vline_args.pop(
                "linewidth", rasterpoint_args["markersize"] / 25
            ),
    ...
    def _validate_lonlat(self, longitude: float, latitude: float) -> Tuple[float]:
        ...
        try:
            ...
            raise TypeError(
                f"Could not convert {longitude} to float. Longitude must be a float."
            )
        ...
        try:
            ...
            raise TypeError(
                f"Could not convert {latitude} to float. Latitude must be a float."
            )

        # Define Madaclim rasters' crs and native units bounds
        madarasters_crs = Constants.MADACLIM_CRS
        madarasters_native_bounds = (
            Constants.MADACLIM_RASTERS_BOUNDS
        )  # (298000.0, 7155000.0, 1101000.0, 8683000.0)
        raster_min_x, raster_max_x = (
            madarasters_native_bounds[0],
            madarasters_native_bounds[2],
        )
        raster_min_y, raster_max_y = (
            madarasters_native_bounds[1],
            madarasters_native_bounds[3],
        )

        if self._source_crs != madarasters_crs:

            # Reproject the lonlat coords to the Madaclim rasters' crs
            mada_transformer = Transformer.from_crs(
                self.source_crs, madarasters_crs, always_xy=True
            )
            reproj_lon, reproj_lat = mada_transformer.transform(longitude, latitude)

            # Get madarasters' bounds in the source_crs' projection
            mada_bounds_reproj_source_crs = mada_transformer.transform_bounds(
                *madarasters_native_bounds, direction="INVERSE"
            )
    ...
    def _validate_lat(self, latitude) -> float:
        ...
        if self._source_crs.is_geographic:
            ...
            transformer = Transformer.from_crs(
                self._source_crs.geodetic_crs, self._source_crs, always_xy=True
            )
        ...
        if not min_lat <= latitude <= max_lat:
            raise ValueError(
                f"{latitude=} is out of bounds for the crs=EPSG:{self._source_crs.to_epsg()}. latitude must be between {min_lat} and {max_lat}"
            )
    ...
    def _validate_lon(self, longitude) -> float:
        ...
        if not min_lon <= longitude <= max_lon:
            raise ValueError(
                f"{longitude=} is out of bounds for the crs=EPSG:{self._source_crs.to_epsg()}. Longitude must be between {min_lon} and {max_lon}"
            )
    ...
    def _get_additional_attributes(self) -> dict:
        ...
        """
        current_attributes = {
            k: v for k, v in self.__dict__.items() if k != "_MadaclimPoint__base_attr"
        }  # Remove the recursiveness of __base_attr
        additional_attributes = {
            k: v
            for k, v in current_attributes.items()
            if k not in self.__base_attr.keys()
        }
        return additional_attributes

    def _construct_point(
        self, latitude: float, longitude: float, source_crs: pyproj.crs.crs.CRS
    ...
    def _construct_geodataframe(self) -> gpd.geopandas:
        ...
        for k, v in self.__dict__.items():
            ...
            if k not in [
                ...
                if k == "_source_crs":  # Display EPSG code for readability
                    ...
                    k == "_sampled_layers"
                ):  # Display number of sampled layers or None if not sampled yet
                    point_attributes[k.lstrip("_")] = [
                        len(v) if self._sampled_layers else v
                    ]
                    ...
                    if self._sampled_layers:
                        point_attributes[k.lstrip("_")] = [
                            len(v) if self._nodata_layers else 0
                        ]
                    else:
        ...
        if self._is_categorical_encoded and self._encoded_categ_layers:
            ...
            madaclim_info = MadaclimLayers()
            possible_categ_labels = madaclim_info.get_categorical_combinations()
            possible_descriptive_categ_labels = (
                madaclim_info.get_categorical_combinations(as_descriptive_keys=True)
            )
...
class MadaclimCollection:
    ...
    def populate_from_csv(
        cls, csv_file: Union[str, pathlib.Path]
    ) -> "MadaclimCollection":
        ...
        madapoint_required_args, madapoint_default_crs_arg = (
            MadaclimPoint.get_args_names()
        )
        ...
        with open(csv_file, newline="") as f:
            ...
            missing_args = [
                req_arg
                for req_arg in madapoint_required_args
                if req_arg not in col_names
            ]
            if len(missing_args) > 0:
                raise ValueError(
                    f"csv file headers are missing the following required args to construct the MadaclimPoint objects:\n{missing_args}"
                )

            # Warn for default EPSG if not provided
            if madapoint_default_crs_arg not in col_names:
                print(
                    f"Warning! No {madapoint_default_crs_arg} column in the csv. Using the default value of EPSG:{madapoint_default_crs_val}..."
                )

            # Replace incompatible attribute names
            sanitized_col_names = [
                MadaclimCollection.sanitize_attr_name(col_name)
            ...
            for row in csv_data:
                ...
                if (
                    madapoint_default_crs_arg not in row
                    or not row[madapoint_default_crs_arg]
                ):
    ...
    def populate_from_df(cls, df: pd.DataFrame) -> "MadaclimCollection":
        ...
        missing_args = [
            req_arg
            for req_arg in madapoint_required_args
            if req_arg not in df_copy.columns
        ]
        if len(missing_args) > 0:
            raise ValueError(
                f"df is missing the following required args to construct the MadaclimPoint objects:\n{missing_args}"
            )

        # Warn for default EPSG if not provided
        if madapoint_default_crs_arg not in df_copy.columns:
            print(
                f"Warning! No {madapoint_default_crs_arg} column in the df. Using the default value of EPSG:{madapoint_default_crs_val}..."
            )

        # Sanitize col names for attrs + construct points to add to collection
        df_copy = df_copy.rename(
            columns={
                col: MadaclimCollection.sanitize_attr_name(col)
    ...
    def add_points(
        self, madaclim_points: Union[MadaclimPoint, List[MadaclimPoint]]
    ) -> None:
        """
        ...
        Examples:
            ...
            File ".../src/py_madaclim/geoclim/raster_manipulation.py", line 1013, in add_points
        ...
        if isinstance(madaclim_points, list):
            for point in madaclim_points:
                ...
                if not isinstance(point, MadaclimPoint):
                    raise TypeError(
                        f"{point} is not a MadaclimPoint object. Accepted types are a single MadaclimPoint and a list of MadaclimPoint objects."
                    )

                if point in self._all_points:
                    raise ValueError(
                        f"{point} is already in the current MadaclimCollection instance."
                    )

                if point.specimen_id in [
                    mada_pt.specimen_id for mada_pt in self._all_points
                ]:  # specimen_id unique id validation
                    raise ValueError(
                        f"specimen_id={point.specimen_id} already exists inside current MadaclimCollection. Every MadaclimPoint must have a unique id."
                    )
            ...
            if not isinstance(madaclim_points, MadaclimPoint):
                raise TypeError(
                    "The madaclim_point to add is not a MadaclimPoint object."
                )

            if madaclim_points in self._all_points:
                raise ValueError(
                    f"{madaclim_points} is already in the current MadaclimCollection instance."
                )

            if madaclim_points.specimen_id in [
                mada_pt.specimen_id for mada_pt in self._all_points
            ]:  # specimen_id unique id validation
                raise ValueError(
                    f"specimen_id={madaclim_points.specimen_id} already exists inside current MadaclimCollection. Every MadaclimPoint must have a unique id."
                )
    ...
    def remove_points(
        ...
        *,
        ...
        if not len(self._all_points) > 0:
            raise ValueError(
                "No points to delete since the MadaclimCollection is empty."
            )
        ...
        if clear:
            if madaclim_points is not None or indices is not None:
                raise ValueError(
                    "When using 'clear', do not provide 'madaclim_points' or 'indices'."
                )
            else:
        ...
        if madaclim_points is None and indices is None:
            raise ValueError(
                "At least one of madaclim_points or indices must be provided."
            )
        ...
        if madaclim_points is not None:
            ...
            if isinstance(madaclim_points, list):
                for point_to_rm in madaclim_points:
                    if not isinstance(point_to_rm, (MadaclimPoint, str)):
                        raise TypeError(
                            f"{point_to_rm} is not a MadaclimPoint object or str. Accepted types are a single MadaclimPoint, a str, a list of MadaclimPoint objects or a list of str."
                        )

                    if isinstance(point_to_rm, MadaclimPoint):
                        if point_to_rm not in self._all_points:
                            raise ValueError(
                                f"{point_to_rm} does not exists in the current MadaclimCollection instance."
                            )
                        madaclim_points_objects.append(point_to_rm)
                    else:
                        if point_to_rm not in [
                            point.specimen_id for point in self._all_points
                        ]:
                            raise ValueError(
                                f"{point_to_rm} is not a valid MadaclimPoint.specimen_id for all points of the collection."
                            )
                ...
                self._all_points = [
                    point
                    for point in self._all_points
                    if point not in madaclim_points_objects
                    and point.specimen_id not in madaclim_points_ids
                ]
            ...
            # Single object removal
                ...
                if not isinstance(madaclim_points, (MadaclimPoint, str)):
                    raise TypeError(
                        f"{madaclim_points} is not a MadaclimPoint object or str. Accepted types are a single MadaclimPoint, a str, a list of MadaclimPoint objects or a list of str."
                    )

                if isinstance(madaclim_points, MadaclimPoint):
                    if madaclim_points not in self._all_points:
                        raise ValueError(
                            f"{madaclim_points} does not exists in the current MadaclimCollection instance."
                        )
                    madaclim_points_objects.append(madaclim_points)
                else:
                    if madaclim_points not in [
                        point.specimen_id for point in self._all_points
                    ]:
                        raise ValueError(
                            f"{madaclim_points} is not a valid MadaclimPoint.specimen_id for all points of the collection."
                        )
        ...
        if indices is not None:
            if isinstance(indices, list):
                try:
                    indices = [
                        int(index) if index >= 0 else len(self._all_points) + int(index)
                ...
                self._all_points = [
                    point
                    for i, point in enumerate(self._all_points)
                    if i not in indices
                ]
                ...
                try:
                    index = (
                        int(indices)
                        if indices >= 0
                        else len(self._all_points) + int(indices)
                    )
                except:
    ...
    def sample_from_rasters(
        ...
        """
        Samples geoclimatic data from raster files for specified layers at the location
        of each point belonging to the MadaclimCollection's instance.

        Calling this method will also update the `sampled_layers` attributes with the data
        extracted from the layers_to_sample for every point in the collection.
        If sampled data containing 'nodata' values, the `nodata_layers` attribute
        ...
        for point in self._all_points:
            ...
            nodata_layers[point.specimen_id] = (
                point.nodata_layers if point.nodata_layers else None
            )
    ...
    def binary_encode_categorical(self) -> None:
        ...
        for point in self._all_points:
            ...
            encoded_categ[point.specimen_id] = (
                point.encoded_categ_layers
            )  # Extract label: value pairs
            labels = point.encoded_categ_layers.keys()
            encoded_labels.extend(
                label for label in labels if label not in encoded_labels
            )
    ...
    def _construct_geodataframe(self) -> Union[str, gpd.GeoDataFrame]:
        ...
        if not len(self._all_points) > 0:
            ...
            all_points_gdf_cols = [
                list(point.gdf.columns) for point in self._all_points
            ]
            # unique_cols_whole_collection = set().union(*all_points_gdf_cols)
            unique_cols_whole_collection = list(
                dict.fromkeys(sum(all_points_gdf_cols, []))
            )
            ...
            if hasattr(self, "_sampled_layers") and hasattr(
                self, "_nodata_layers"
            ):  # Avoid block-trigger when constructing with points
                has_sampled_layers = self._sampled_layers is not None or any(
                    [point.sampled_layers for point in self._all_points]
                )
                has_not_encoded_layers = not self._is_categorical_encoded or not any(
                    [point.is_categorical_encoded for point in self._all_points]
                )

                # Reorder the cols to keep logical display + sampled_layers at right end
                if has_sampled_layers and has_not_encoded_layers:
                    all_sampled_layers_cols = [
                        list(point.sampled_layers.keys())
                        for point in self._all_points
                        if point.sampled_layers
                    ]
                    unique_sampled_layers_cols = list(
                        dict.fromkeys(sum(all_sampled_layers_cols, []))
                    )
                    unsampled_cols = [
                        col
                        for col in collection_gdf.columns
                        if col not in unique_sampled_layers_cols
                    ]
            ...
            if hasattr(self, "_is_categorical_encoded") and hasattr(
                self, "_encoded_categ_layers"
            ):  # Avoid block-trigger when constructing with points
                has_sampled_layers = self._sampled_layers is not None or any(
                    [point.sampled_layers for point in self._all_points]
                )
                has_encoded_layers = self._is_categorical_encoded or any(
                    [point.is_categorical_encoded for point in self._all_points]
                )
                ...
                if has_sampled_layers and has_encoded_layers:
                    ...
                    madaclim_info = MadaclimLayers()
                    possible_categ_labels = list(
                        madaclim_info.get_categorical_combinations().keys()
                    )
                    possible_descriptive_categ_labels = list(
                        madaclim_info.get_categorical_combinations(
                            as_descriptive_keys=True
                        ).keys()
                    )
                    ...
                    noncateg_sampled_layers_cols = [
                        ele
                        for sublist in all_sampled_layers_cols
                        for ele in sublist
                        if ele
                        not in possible_categ_labels + possible_descriptive_categ_labels
                    ]
                    all_categ_layers_cols = [
                        list(point.encoded_categ_layers.keys())
                        for point in self._all_points
                        if point.is_categorical_encoded
                    ]

                    unique_sampled_layers_cols = list(
                        dict.fromkeys(noncateg_sampled_layers_cols, [])
                    )
                    unique_encoded_layers_cols = list(
                        dict.fromkeys(sum(all_categ_layers_cols, []))
                    )

                    non_sampled_categ_cols = [
                        col
                        for col in collection_gdf.columns
                        if col
                        not in unique_sampled_layers_cols + unique_encoded_layers_cols
                    ]
                    reordered_cols = (
                        list(non_sampled_categ_cols)
                        + unique_sampled_layers_cols
                        + unique_encoded_layers_cols
                    )
    ...
    def plot_on_layer(self, layer: Union[str, int], **kwargs) -> None:
        ...
        # Validate object's raster-sampled state
        if (
            self._MadaclimCollection__clim_raster is None
            or self._MadaclimCollection__env_raster is None
        ):
            raise AttributeError(
                "No 'clim_raster' and 'env_raster' found, use 'sample_from_rasters' prior to setup a reference to the raster files location."
            )
        ...
        mada_rasters = MadaclimRasters(
            self._MadaclimCollection__clim_raster, self._MadaclimCollection__env_raster
        )
        mada_info = layer_name_range_validation(
            mada_rasters.clim_raster, mada_rasters.env_raster, layer
        )
        ...
        layer_number = mada_info.fetch_specific_layers(layer)["layer_number"].values[0]
        layer_possible_labels = mada_info.get_layers_labels(
            layer_number
        ...
        if not any(
            [
                layer_to_plot in sampled_layers_labels
                for layer_to_plot in layer_possible_labels
            ]
        ):
            raise ValueError(
        ...
        if (
            "palette" in plot_cfg.rasterpoint_args
            and "palette" in plot_cfg.barplot_args
        ):
            raise ValueError(
                "Only set a palette for 'rasterpoint_palette' or 'barplot_palette"
            )
